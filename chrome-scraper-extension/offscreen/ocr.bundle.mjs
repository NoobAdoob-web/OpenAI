...
async function Vr(n = {}) {
  ...
  var H, ne, pe, B, W, re, me, A, K, je = false;
  function ee() {
    var e = ge.buffer;
...
// src/ppocr-offscreen.mjs
var BASE = chrome.runtime.getURL("ocr/");
Y.wasm.wasmPaths = BASE;
Y.wasm.numThreads = 1;
try {
  Y.wasm.proxy = false;
} catch (_) {
...
function getDet() {
  if (!detSess) detSess = ts.create(BASE + "ch_PP-OCRv4_det_infer.onnx", { executionProviders: ["wasm"] });
  return detSess;
}
function getRec() {
  if (!recSess) recSess = ts.create(BASE + "ch_PP-OCRv4_rec_infer.onnx", { executionProviders: ["wasm"] });
  return recSess;
}
...
async function detLines(img, W, H) {
  ...
  const out = (await sess.run({ x: new de("float32", data, [1, 3, H2, W2]) }))["sigmoid_0.tmp_0"];
...
function cropRGBA(src, W, H, b) {
  ...
  for (let y = 0; y < b.h; y++) for (let x = 0; x < b.w; x++) {
    const s = ((b.y + y) * W + (b.x + x)) * 4, d = (y * b.w + x) * 4;
    out[d] = src[s];
    out[d + 1] = src[s + 1];
    out[d + 2] = src[s + 2];
    out[d + 3] = 255;
...
function decodeCTC(dict, idx, prob) {
  ...
  for (let i = 0; i < idx.length; i++) {
    if (idx[i] === 0) continue;
    if (i > 0 && idx[i - 1] === idx[i]) continue;
    cs2.push(dict[idx[i] - 1]);
    cf.push(prob[i]);
  }
...
async function recognize(dict, cr, cw, ch) {
  ...
  const out = (await sess.run({ x: new de("float32", Float32Array.from([...B, ...G2, ...R]), [1, 3, Hh, Wr2]) }))["softmax_11.tmp_0"];
  const predLen = out.dims[2];
  const idx = [], prob = [];
...
async function runOcrVideo(url, maxFrames) {
  ...
  try {
    await new Promise((res, rej) => {
      ...
      setTimeout(() => rej(new Error("video load timeout")), 2e4);
    });
    ...
    for (let i = 0; i < n; i++) {
      ...
      try {
        ...
        throw new Error("frame read blocked: " + e.name);
      }
...
chrome.runtime.onMessage.addListener((msg, _sender, sendResponse) => {
  ...
  if (msg.action === "ocrVideo") {
    runOcrVideo(msg.url, msg.maxFrames).then((r) => sendResponse({ ok: true, text: r.text, conf: r.conf, frames: r.frames, duration: r.duration })).catch((e) => sendResponse({ ok: false, error: String(e).slice(0, 180) }));
    return true;
  }
  if (msg.action === "ocrDispose") {
    detSess = recSess = null;

...
// Get the exact posting date (+ duration) for a small set of posts by OPENING
// each in a real background tab and scraping the rendered page — the same
// deep-scrape path as the per-post enrichment. No fetch()/guesswork: this is
// what makes the first/last-post date range accurate on Instagram etc.
async function getPostMeta(urls) {
  const out = {};
  for (const url of urls) {
    if (!url) continue;
    try {
      const d = await openAndExtract(url, { date: true });
      out[url] = { date: normalizeDate(d && d.Date), durationSec: (d && d.DurationSec) || '' };
    } catch (e) {
      out[url] = { date: '', durationSec: '', error: String(e).slice(0, 100) };
    }
  }
  return out;
}

// Normalize any scraped date string to YYYY-MM-DD.
function normalizeDate(s) {
  if (!s) return '';
  const m = String(s).match(/(\d{4})-(\d{2})-(\d{2})/);
  if (m) return `${m[1]}-${m[2]}-${m[3]}`;
  const t = Date.parse(s);
  if (!isNaN(t)) { try { return new Date(t).toISOString().slice(0, 10); } catch (_) {} }
  return '';
}
